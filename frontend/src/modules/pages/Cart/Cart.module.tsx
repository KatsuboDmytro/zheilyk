...
import { Basket, CartType, Order } from '../../../types/Cart'
...
export const Cart: React.FC = () => {
    ...
    const fetch = async () => {
      ...
      if (accessToken !== null) {
        ...
        try {
          ...
          if (response && response[0] && response[0].basket_items) {
            setBasket(response[0].basket_items)
          } else {
            setBasket([])
          }
          setIsLoading(false);
  ...
  const updateCartOnServer = async (updatedBasket: Basket[]) => {
    try {
      const newCart: CartType = { ...cart[0], basket_items: updatedBasket };
			const response = await cartService.updateCart(newCart, 'en');
			dispatch(setCart(response));
		} catch (error) {
	...
	const handlePlusQuantity = (id: number) => {
		const updatedBasket = basket.map((good) => {
			if (good.id === id) {
		...
		updateCartOnServer(updatedBasket);
	};
  ...
  const handleDeleteItem = (id: number) => {
    const updatedBasket = basket.filter((good) => good.id !== id);
    setBasket(updatedBasket);
    updateCartOnServer(updatedBasket);
  }
