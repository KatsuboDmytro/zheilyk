...
const OrderSummary: React.FC<OrderSummaryProps> = ({ handleCheckout, cart }) => {
  ...
  return (
    <aside className="checkout__goods">
      <div className="checkout__titles">
        ...
        <h4 className="checkout__subtitle">{totalItems} товарів</h4>
      </div>
          ...
          <React.Fragment key={item.id}>
            {cart.map((good: any) => (
              <article key={good.id} className="account__order--card account__order--cart">
                <img src={`img{item.images}`} alt={good.item} className="account__order--card-img" />
      ...
      <div className="checkout__buttons">
        ...
        <button className="account__button checkout__button" onClick={handleCheckout}>
